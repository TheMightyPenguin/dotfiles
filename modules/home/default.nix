...

{
  ...
  home = {
    ...
    # Toolchains that install into $HOME on both platforms. The tools
    # themselves come from nix; these are the dirs their *global installs*
    # land in, which nix doesn't own.
    sessionPath = [
      "$HOME/bin"
      "$HOME/.local/bin"
      "$HOME/.cargo/bin" # rustup toolchains
      "$HOME/.bun/bin" # bun global installs
      "$HOME/.deno/bin" # deno global installs
    ];
  };
